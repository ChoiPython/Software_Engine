...
from control.menu_add_control import *  # 메뉴 등록 컨트롤러 import
...
class Menu_add:
    ...
    def run(self):
        ...
        if len(self.req_opt) == 0:
            ...
            for i in self.req_opt:  # 등록되어 있는 옵션들 출력
                Label(self.req_opt_frame, text=i[0] + '\t' + i[1], width=20, height=1, relief="solid", borderwidth=1, bg="white").pack(side="top", pady=10)
        ...
        if len(self.add_opt) == 0:
            ...
            for i in self.add_opt:  # 등록되어 있는 옵션들 출력
                Label(self.add_opt_frame, text=i[0] + '\t' + i[1], width=20, height=1, relief="solid", borderwidth=1, bg="white").pack(side="top", pady=10)
    ...
    def submit(self):
        ...
        # 메뉴 등록 컨트롤러 호출
        # print(self.menu_data)
        result_menu = self.add_control.menu_add(self.menu_data)
        result_opt = self.add_control.Option_add(self.menu_data, self.req_opt, self.add_opt)
